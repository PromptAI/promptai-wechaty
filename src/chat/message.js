...
exports.onLogin = async function onLogin(user) {
  log.info("StarterBot", "%s login", user);
  username = user.name();
  CALL_BOT_CONFIG.push(username);
};

exports.onLogout = async function onLogout(user) {
  log.info("StarterBot", "%s logout", user);
  username = user.name();
};

exports.onMessage = async function onMessage(message, bot) {
  //不处理非文本内容
...
exports.initChat = async function initChat(talkerId, message, bot) {
...
exports.encodeAttachmentText = function encodeAttachmentText(value) {
...
exports.processMessage = async function processMessage(reply, message, bot) {
  const answers =
...
exports.chatExpires = function chatExpires(chat) {
...
exports.needReply = function needReply(msg) {
  for (let i = 0; i < CALL_BOT_CONFIG.length; i++) {
    if (msg.indexOf(CALL_BOT_CONFIG[i]) !== -1) {
      msg = msg.replace(CALL_BOT_CONFIG[i], "");
